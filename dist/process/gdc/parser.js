...
export class Tokenizer {
    constructor(gdcFile) {
        this.current = 0;
        this.current_line = 1;
        this.pending_indents = 0;
        this.last_token_was_newline = false;
        this.indent_stack = [];
        this.indent_stack_stack = [];
...
export class BinaryParser {
    constructor(tokenizer, script_path) {
        ...
        this.in_lambda = false;
        this.lambda_ended = false;
        this.annotation_stack = [];
        this.multiline_stack = [];
        registerAnnotations();
    }
    /*
    get current(): Token {
  ...
  */
    ...
    consume(p_token_type, p_error_message) {
        ...
        while (can_have_class_or_extends) {
            switch (this.current.type) {
                ...
                case TokenType.LITERAL:
                    const literal = this.current.literal;
    ...
    parse_class(_p_is_static) {
        const n_class = new ClassNode();
        const previous_class = this.current_class;
        ...
        const multiline = this.match(TokenType.NEWLINE);
    ...
    parse_class_member(p_parse_function, p_target, p_member_kind, p_is_static = false) {
        ...
        if (member.identifier != null) {
            if (member.identifier.name.length) { // Enums may be unnamed.
                if (this.current_class.members_indices.has(member.identifier.name)) {
                    push_error(`(${p_member_kind.toUpperCase()} "${member.identifier.name}" has the same name as a previously declared ${this.current_class.get_member(member.identifier.name).get_type_name()}.)`);
                }
                else {
    ...
    parse_class_body(p_is_multiline) {
        const current_class = tryOrThrow(this.current_class);
        let class_end = false;
    ...
    parse_variable(p_is_static, p_allow_property = true) {
    ...
    parse_property(p_variable, p_need_indent) {
        ...
        let getter_used = false;
        let setter_used = false;
    ...
    parse_property_setter(p_variable) {
        switch (p_variable.property) {
            case VariableNode_PropertyStyle.PROP_INLINE: {
                ...
                const parameter = new ParameterNode();
    ...
    parse_property_getter(p_variable) {
        switch (p_variable.property) {
            case VariableNode_PropertyStyle.PROP_INLINE: {
                const _function = new FunctionNode();
    ...
    parse_constant(_p_is_static) {
        const constant = new ConstantNode();
    ...
    parse_signal(_p_is_static) {
        ...
        if (this.check(TokenType.PARENTHESIS_OPEN)) {
            ...
            do {
                ...
                const parameter = this.parse_parameter();
                if (parameter == null) {
    ...
    parse_enum(_p_is_static) {
        const enum_node = new EnumNode();
        let named = false;
        if (this.match(TokenType.IDENTIFIER)) {
        ...
        do {
            ...
            if (this.consume(TokenType.IDENTIFIER, '(Expected identifier for enum key.)')) {
                ...
                if (this.match(TokenType.EQUAL)) {
                    const value = this.parse_expression(false);
                    if (value == null) {
    ...
    parse_function_signature(p_function, p_body, p_type) {
    ...
    parse_annotation(p_valid_targets) {
        const annotation = new AnnotationNode();
        annotation.name = this.previous.literal;
    ...
    parse_suite(p_context, p_suite = null, p_for_lambda = false) {
        const suite = p_suite != null ? p_suite : new SuiteNode();
        ...
        do {
            ...
            switch (statement.type) {
                case Type.VARIABLE: {
                    ...
                    const local = this.current_suite.get_local(variable.identifier.name);
                ...
                case Type.CONSTANT: {
                    const constant = statement;
                    const local = this.current_suite.get_local(constant.identifier.name);
                    if (local.type != SuiteNode_Local_Type.UNDEFINED) {
                        let name;
    ...
    parse_statement() {
        ...
        if (this.current.type != TokenType.ANNOTATION) {
            while (this.annotation_stack.length != 0) {
                const last_annotation = this.annotation_stack.slice(-1)[0];
        ...
        switch (this.current.type) {
            ...
            case TokenType.ANNOTATION: {
                this.advance();
                const annotation = this.parse_annotation(AnnotationInfo_TargetKind.STATEMENT);
                if (annotation != null) {
                    this.annotation_stack.push(annotation);
                }
                break;
            ...
            default: {
                ...
                const expression = this.parse_expression(true); // Allow assignment here.
                let has_ended_lambda = false;
                if (expression == null) {
                    if (this.in_lambda) {
    ...
    parse_break() {
        ...
        const break_node = new BreakNode();
    ...
    parse_continue() {
        ...
        const cont = new ContinueNode();
        this.end_statement('("continue")');
        return cont;
    }
    parse_for() {
        const n_for = new ForNode();
    ...
    parse_match() {
        const match_node = new MatchNode();
        ...
        let all_have_return = true;
        let have_wildcard = false;
        const match_branch_annotation_stack = [];
        while (!this.check(TokenType.DEDENT) && !this.is_at_end()) {
            ...
            const branch = this.parse_match_branch();
            if (branch == null) {
                this.advance();
                continue;
            }
            match_branch_annotation_stack.forEach((annotation) => {
    ...
    parse_match_branch() {
        const branch = new MatchBranchNode();
        let has_bind = false;
        do {
            const pattern = this.parse_match_pattern();
            if (pattern == null) {
                continue;
            }
    ...
    parse_match_pattern(p_root_pattern = null) {
        ...
        switch (this.current.type) {
            case TokenType.VAR:
                {
                    ...
                    const root_pattern = p_root_pattern == null ? pattern : p_root_pattern;
            ...
            case TokenType.BRACKET_OPEN: {
                ...
                do {
                    ...
                    const sub_pattern = this.parse_match_pattern(p_root_pattern != null ? p_root_pattern : pattern);
                    if (sub_pattern == null) {
                        continue;
                    }
                ...
                do {
                    ...
                    if (this.match(TokenType.PERIOD_PERIOD)) {
                        ...
                        if (pattern.rest_used) {
                            ...
                            const sub_pattern = new PatternNode();
                        ...
                        const key = this.parse_expression(false);
                        if (key == null) {
    ...
    parse_precedence(p_precedence, p_can_assign, p_stop_on_assign = false) {
        switch (this.current.type) {
    ...
    parse_expression(p_can_assign, p_stop_on_assign = false) {
        return this.parse_precedence(Precedence.PREC_ASSIGNMENT, p_can_assign, p_stop_on_assign);
    }
    parse_identifier(_p_previous_operand = null, _p_can_assign = false) {
        ...
        if (current_suite != null && current_suite.has_local(identifier.name)) {
            const declaration = current_suite.get_local(identifier.name);
                ...
                case SuiteNode_Local_Type.CONSTANT:
                    {
                        ...
                        const dec = identifier.node_source = declaration.value;
                        dec.usages++;
                    }
                    break;
    ...
    parse_literal(_p_previous_operand, _p_can_assign) {
    ...
    parse_unary_operator(_p_previous_operand, _p_can_assign) {
        const op_type = this.previous.type;
        const operation = new UnaryOpNode();
        switch (op_type) {
    ...
    parse_binary_not_in_operator(p_previous_operand, p_can_assign) {
        ...
        const in_operation = this.parse_binary_operator(p_previous_operand, p_can_assign);
        operation.operation = UnaryOpNode_OpType.OP_LOGIC_NOT;
        operation.variant_op = VariantOperator.OP_NOT;
        operation.operand = in_operation;
        return operation;
    }
    parse_binary_operator(p_previous_operand, _p_can_assign) {
        const op = this.previous;
        const operation = new BinaryOpNode();
    ...
    parse_ternary_operator(p_previous_operand, _p_can_assign) {
        // Only one ternary operation exists, so no abstraction here.
        const operation = new TernaryOpNode();
    ...
    parse_assignment(p_previous_operand, p_can_assign) {
        if (!p_can_assign) {
        ...
        const assignment = new AssignmentNode();
    ...
    parse_await(_p_previous_operand, _p_can_assign) {
        const _await = new AwaitNode();
        const element = this.parse_precedence(Precedence.PREC_AWAIT, false);
        if (element == null) {
    ...
    parse_array(_p_previous_operand, _p_can_assign) {
        ...
        if (!this.check(TokenType.BRACKET_CLOSE)) {
            do {
                ...
                const element = this.parse_expression(false);
                if (element == null) {
    ...
    parse_dictionary(_p_previous_operand, _p_can_assign) {
        const dictionary = new DictionaryNode();
        let decided_style = false;
        if (!this.check(TokenType.BRACE_CLOSE)) {
            do {
                if (this.check(TokenType.BRACE_CLOSE)) {
                    // Allow for trailing comma.
                ...
                // Key.
                const key = this.parse_expression(false, true); // Stop on "=" so we can this.check for Lua table style.
                if (key == null) {
                ...
                switch (dictionary.style) {
                    case DictionaryNode_Style.LUA_TABLE:
                        ...
                        if (key != null && key.type == Type.LITERAL && key.value.type_s != 'STRING') {
                            push_error('(Expected identifier or string as Lua-style dictionary key (e.g "{ key = value }").)');
                            this.advance();
                            break;
                        }
                        ...
                        if (key != null) {
                            ...
                            if (key.type == Type.IDENTIFIER) {
                                ...
                                key.reduced_value = key.value.val.toString();
                            }
                        }
                        break;
    ...
    parse_grouping(_p_previous_operand, _p_can_assign) {
        const grouped = this.parse_expression(false);
        this.pop_multiline();
        if (grouped == null) {
    ...
    parse_attribute(p_previous_operand, _p_can_assign) {
        const attribute = new SubscriptNode();
    ...
    parse_subscript(p_previous_operand, _p_can_assign) {
        const subscript = new SubscriptNode();
    ...
    parse_cast(p_previous_operand, _p_can_assign) {
        const cast = new CastNode();
    ...
    parse_call(p_previous_operand, _p_can_assign) {
        ...
        if (this.previous.type == TokenType.SUPER) {
            ...
            if (call.callee == null) {
                ...
                call.function_name = call.callee.name;
            }
            else if (call.callee.type == Type.SUBSCRIPT) {
                const attribute = call.callee;
    ...
    parse_get_node(_p_previous_operand, _p_can_assign) {
        ...
        if (this.check(TokenType.LITERAL)) {
            if (this.current.literal.type_s != 'STRING') {
                push_error(vformat('(Expected node path as string or identifier after "%s".)', this.previous.type));
                return null;
            }
        }
        const get_node = new GetNodeNode();
        // Store the last item in the path so the parser knows what to expect.
        // Allow allows more specific error messages.
        let PathState;
        (function (PathState) {
            PathState[PathState["PATH_STATE_START"] = 0] = "PATH_STATE_START";
            PathState[PathState["PATH_STATE_SLASH"] = 1] = "PATH_STATE_SLASH";
            PathState[PathState["PATH_STATE_PERCENT"] = 2] = "PATH_STATE_PERCENT";
            PathState[PathState["PATH_STATE_NODE_NAME"] = 3] = "PATH_STATE_NODE_NAME";
        })(PathState || (PathState = {}));
        ;
        let path_state = PathState.PATH_STATE_START;
        ...
        do {
            ...
            if (this.match(TokenType.LITERAL)) {
                if (this.previous.literal.type_s != 'STRING') {
                    let previous_token = '';
                    switch (path_state) {
                ...
                get_node.full_path += this.previous.literal.val;
    ...
    parse_preload(_p_previous_operand, _p_can_assign) {
        const preload = new PreloadNode();
    ...
    parse_lambda(_p_previous_operand, _p_can_assign) {
    ...
    parse_type_test(p_previous_operand, _p_can_assign) {
        // x is not int
        ...
        const type_test = new TypeTestNode();
    ...
    parse_yield(_p_previous_operand, _p_can_assign) {
        push_error('("yield" was removed in Godot 4. Use "await" instead.)');
        return null;
    }
    parse_invalid_token(p_previous_operand, _p_can_assign) {
