...
import { parse_variant } from "./variant.js";
const FORMAT_VERSION = 6;
var FORMAT_FLAGS;
(function (FORMAT_FLAGS) {
    FORMAT_FLAGS[FORMAT_FLAGS["FORMAT_FLAG_NAMED_SCENE_IDS"] = 1] = "FORMAT_FLAG_NAMED_SCENE_IDS";
    FORMAT_FLAGS[FORMAT_FLAGS["FORMAT_FLAG_UIDS"] = 2] = "FORMAT_FLAG_UIDS";
    FORMAT_FLAGS[FORMAT_FLAGS["FORMAT_FLAG_REAL_T_IS_DOUBLE"] = 4] = "FORMAT_FLAG_REAL_T_IS_DOUBLE";
    FORMAT_FLAGS[FORMAT_FLAGS["FORMAT_FLAG_HAS_SCRIPT_CLASS"] = 8] = "FORMAT_FLAG_HAS_SCRIPT_CLASS";
    // Amount of reserved 32-bit fields in resource header
    FORMAT_FLAGS[FORMAT_FLAGS["RESERVED_FIELDS"] = 11] = "RESERVED_FIELDS";
...
ResourceUID.INVALID_ID = -1n;
export async function try_open_bin_resource(res_path, arrayBuffer, p_no_resource, p_keep_uuid_paths) {
    ...
    const big_endian = !!dataView.getUint32(read_offset);
    read_offset += 4;
    // @ts-ignore
    const use_real64 = !!dataView.getUint32(read_offset);
    read_offset += 4;
    ...
    if (ver_format > FORMAT_VERSION || ver_major > VERSION_MAJOR) {
        ERR_FAIL_V_MSG('ERR_FILE_UNRECOGNIZED', `File '${res_path}' can't be loaded, as it uses a format version (${ver_format}) or engine version (${ver_major}.${ver_minor})` +
