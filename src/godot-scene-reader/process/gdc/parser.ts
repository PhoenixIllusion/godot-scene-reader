import { Constant, Token } from "../../parse/binary/gdc";
...
import { AnnotationInfo_TargetKind, AnnotationNode, ArrayNode, AssertNode, AssignmentNode, AssignmentOperation, AwaitNode, BinaryOpNode, BinaryOpType, BreakNode, BreakpointNode, ClassMemberSource, ClassNode, ConstantNode, ContinueNode, DictionaryNode, DictionaryNode_Style, EnumNode, EnumValue, ExpressionNode, ForNode, FunctionNode, IdentifierNode, IdentifierNode_Source, IfNode, LambdaNode, LiteralNode, MatchBranchNode, MatchNode, Node, ParameterNode, PassNode, PatternNode, PatternNode_Type, ReturnNode, SelfNode, SignalNode, SuiteNode, SuiteNode_Local, SuiteNode_Local_Type, TernaryOpNode, Type, TypeNode, UnaryOpNode, UnaryOpNode_OpType, VariableNode, VariableNode_PropertyStyle, VariantOperator, WhileNode } from "./type";
...
export class BinaryParser {
  ...
  constructor(private tokens: Token[], public script_path: string) { }
  get current(): Token {
    return {
  ...
  parse_precedence(p_precedence: Precedence, p_can_assign: boolean, p_stop_on_assign: boolean): ExpressionNode | null {
    ...
    this.advance();
    const prefix_rule = get_rule(token_type).prefix;
    if (prefix_rule == null) {
      throw new Error();
    }
    ...
    while (p_precedence <= get_rule(this.current.type).precedence) {
      if (previous_operand == null || (p_stop_on_assign && this.current.type == TokenType.EQUAL) || this.lambda_ended) {
        return previous_operand;
      }
      token = this.advance();
      const infix_rule = get_rule(token.type).infix;
      if (infix_rule == null) {
        throw new Error();
      }
  ...
  parse_annotation(): AnnotationNode | null {
  ...
  private clear_unused_annotations() {
  ...
  parse_class_member<T extends Exclude<ClassMemberSource, AnnotationNode | EnumValue | undefined>>(p_parse_function: (is_static: boolean) => T | null, p_target: AnnotationInfo_TargetKind, p_member_kind: string, p_is_static: boolean = false) {
    ...
    if (member.identifier != null) {
      if (!(member.identifier.name.length == 0)) { // Enums may be unnamed.
  ...
  parse_function_signature(p_function: FunctionNode, p_body: SuiteNode, p_type: string): void {
    if (!this.check(TokenType.PARENTHESIS_CLOSE) && !this.is_at_end()) {
      ...
      do {
        ...
        const parameter: ParameterNode = this.parse_parameter();
        if (parameter == null) {
          break;
        }
        if (parameter.initializer != null) {
  ...
  parse_enum(p_is_static: boolean): EnumNode | null {
    ...
    this.consume(TokenType.BRACE_OPEN, vformat('(Expected "{" after %s.)", named ? "enum name" : R"("enum")'));
    ...
    while (!class_end && !this.is_at_end()) {
      ...
      switch (token.type) {
        ...
        case TokenType.ANNOTATION: {
          ...
          const annotation = this.parse_annotation();
          if (annotation != null) {
        ...
        if (annotation != null) {
          if (annotation.applies_to(AnnotationInfo_TargetKind.CLASS)) {
            ...
            if (annotation.name == ("@tool") || annotation.name == ("@icon")) {
              ...
              annotation.apply(this, head, null); // `head.outer == nullptr`.
            } else {
  ...
  parse_literal(p_previous_operand: ExpressionNode, p_can_assign: boolean): ExpressionNode | null {
    if (this.previous.type != TokenType.LITERAL) {
      ...
      ERR_FAIL_V_MSG("Parser bug: parsing literal node without literal token.");
    }
  ...
  parse_self(p_previous_operand: ExpressionNode, p_can_assign: boolean): ExpressionNode | null {
    ...
    const self: SelfNode = new SelfNode();
    
    self.current_class = this.current_class;
    return self;
  }
  ...
  parse_binary_operator(p_previous_operand: ExpressionNode, p_can_assign: boolean): ExpressionNode | null {
    ...
    const precedence = get_rule(op.type).precedence + 1;
    ...
    if (operation.right_operand == null) {
      push_error(vformat('(Expected expression after "%s" operator.)', op.get_name()));
    }
  ...
  parse_await(p_previous_operand: ExpressionNode, p_can_assign: boolean): ExpressionNode | null {
    const await: AwaitNode = new AwaitNode();
      ...
      do {
        ...
        switch (dictionary.style) {
          case DictionaryNode_Style.LUA_TABLE:
            ...
            if (key != null && key.type == Type.LITERAL && skey).value.get_type() != Variant::STRING) {
              push_error('(Expected identifier or string as Lua-style dictionary key (e.g "{ key = value }").)');
              this.advance();
              break;
            }
            ...
            break;
          case DictionaryType.PYTHON_DICT:
            if (!this.match(TokenType.COLON)) {
              if (this.match(TokenType.EQUAL)) {
                push_error('(Expected ":" after dictionary key. Mixing dictionary styles is not allowed.)');
                this.advance(); // Consume wrong separator anyway.
        ...
        // Value.
        value: ExpressionNode = this.parse_expression(false);
        if (value == null) {
  ...
  this.parse_grouping: ExpressionNode(p_previous_operand: ExpressionNode, p_can_assign: boolean) {
    grouped: ExpressionNode = this.parse_expression(false);
    pop_multiline();
    if (grouped == null) {
  ...
  this.parse_attribute: ExpressionNode(p_previous_operand: ExpressionNode, p_can_assign: boolean) {
    attribute: SubscriptNode = new SubscriptNode();
    reset_extents(attribute, p_previous_operand);
    update_extents(attribute);
  
    if (for_completion) {
      is_builtin: boolean = false;
      if (p_previous_operand && p_previous_operand.type == Type.IDENTIFIER) {
        const id: IdentifierNode = static_cast<const IdentifierNode *>(p_previous_operand);
  ...
  this.parse_subscript: ExpressionNode(p_previous_operand: ExpressionNode, p_can_assign: boolean) {
    subscript: SubscriptNode = new SubscriptNode();
  ...
  #ifdef TOOLS_ENABLED
    if (subscript.index != null && subscript.index.type == Type.LITERAL) {
  ...
  this.parse_cast: ExpressionNode(p_previous_operand: ExpressionNode, p_can_assign: boolean) {
    cast: CastNode = new CastNode();
  ...
  this.parse_call: ExpressionNode(p_previous_operand: ExpressionNode, p_can_assign: boolean) {
    call: CallNode = new CallNode();
    reset_extents(call, p_previous_operand);
  
    if (previous.type == TokenType.SUPER) {
      // Super call.
      call.is_super = true;
      push_multiline(true);
      if (this.match(TokenType.PARENTHESIS_OPEN)) {
        ...
        identifier: IdentifierNode = this.parse_identifier();
      ...
      if (call.callee == null) {
        ...
        attribute: SubscriptNode = static_cast<SubscriptNode *>(call.callee);
    ...
    do {
      ...
      argument: ExpressionNode = this.parse_expression(false);
      if (argument == null) {
        push_error('(Expected expression as the function argument.)');
      } else {
        call.arguments.push_back(argument);
  
        if (argument.type == Type.LITERAL) {
  ...
  this.parse_get_node: ExpressionNode(p_previous_operand: ExpressionNode, p_can_assign: boolean) {
    // We want code completion after a DOLLAR even if the this.current code is invalid.
    
  
    if (!this.current.is_node_name() && !this.check(TokenType.LITERAL) && !this.check(TokenType.SLASH) && !this.check(TokenType.PERCENT)) {
      push_error(vformat('(Expected node path as string or identifier after "%s".)', previous.get_name()));
      return null;
    }
  
    if (this.check(TokenType.LITERAL)) {
      if (this.current.literal.get_type() != Variant::STRING) {
        push_error(vformat('(Expected node path as string or identifier after "%s".)', previous.get_name()));
        return null;
      }
    }
  
    get_node: GetNodeNode = new GetNodeNode();
    ...
    if (previous.type == TokenType.DOLLAR) {
    ...
    do {
      if (previous.type == TokenType.PERCENT) {
      ...
      } else if (previous.type == TokenType.SLASH) {
      ...
      } else if (this.current.is_node_name()) {
        this.advance();
  ...
  this.parse_preload: ExpressionNode(p_previous_operand: ExpressionNode, p_can_assign: boolean) {
    preload: PreloadNode = new PreloadNode();
